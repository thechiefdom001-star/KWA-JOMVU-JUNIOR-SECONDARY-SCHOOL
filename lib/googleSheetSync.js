    ...
    async fetchAll() {
        ...
        try {
            ...
            try {
                ...
                if (data.students && Array.isArray(data.students)) {
                    data.students = data.students.map(s => {
                        if (s.selectedFees && typeof s.selectedFees === 'string') {
                            // If it contains Java object notation, replace with default
                            if (s.selectedFees.includes('java.lang.Object') || s.selectedFees.startsWith('[L')) {
                                console.log('🧹 Cleaned corrupted selectedFees for student:', s.name, s.selectedFees);
                                s.selectedFees = 't1,t2,t3'; // Default format that parseSelectedFees can handle
                            }
                        }
                        return s;
                    });
                ...
                console.log('Google Sheet response - students:', data.students?.length, 'assessments:', data.assessments?.length);
                return {
                    success: true,
