export const Storage = {
        ...
        try {
            ...
            return {
                ...
                settings: { 
                    ...
                    ...settings,
                    grades: allGrades,
                    streams: settings.streams || this.defaultData.settings.streams,
    ...
    mergeData(existingData, incomingData, type) {
        ...
        switch (type) {
            ...
            case 'all': {
                // Smart merge: keep existing + add new from Google (no duplicates)
                const existingStudents = existingData.students || [];
                const incomingStudents = incomingData.students || [];
                const studentIds = new Set(existingStudents.map(s => s.admissionNo));
                const mergedStudents = [
                    ...existingStudents,
                    ...incomingStudents.filter(s => !studentIds.has(s.admissionNo))
                ];

                const existingAssessments = existingData.assessments || [];
                const incomingAssessments = incomingData.assessments || [];
                const assessmentKeys = new Set(existingAssessments.map(a => 
                    `${a.studentId}-${a.subject}-${a.term}-${a.examType}-${a.academicYear}`
                ));
                const mergedAssessments = [
                    ...existingAssessments,
                    ...incomingAssessments.filter(a => !assessmentKeys.has(
                        `${a.studentId}-${a.subject}-${a.term}-${a.examType}-${a.academicYear}`
                    ))
                ];
                ...
                const attendanceKeys = new Set(existingAttendance.map(a => 
                    `${a.studentId}-${a.date}`
                ));
                const mergedAttendance = [
                    ...existingAttendance,
                    ...incomingAttendance.filter(a => !attendanceKeys.has(`${a.studentId}-${a.date}`))
                ];
