export const Storage = {
    ...
    save(data) {
        try {
            ...
            const storage = this.getStorage();
            storage.setItem(this.key, JSON.stringify(dataToSave));
            console.log('Data saved successfully, students:', dataToSave.students?.length, 'feeStructures:', dataToSave.settings.feeStructures?.length);
        } catch (e) {
    ...
    parseSelectedFees(fees) {
        ...
        // Convert string to array - handles both "t1,t2,t3" and "t1" formats
        if (typeof fees === 'string') {
            return fees.split(',').map(f => f.trim()).filter(f => f);
        }
        return ['t1', 't2', 't3'];
    },
    ...
    replaceWithGoogleData(existingData, googleData) {
        ...
        const googleStudents = (googleData.students || []).filter(s => s.admissionNo);
        const uniqueStudents = [];
        const seenAdmissions = new Set();
        
        for (const student of googleStudents) {
            if (student.admissionNo && !seenAdmissions.has(student.admissionNo)) {
